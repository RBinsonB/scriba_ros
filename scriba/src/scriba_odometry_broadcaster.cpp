...
#include "scriba_msgs/motion_odom.h"
...
private:
	...
	ros::Publisher motion_pub;					//Motion publisher
	ros::Subscriber odom_data_sub;				//Odometry data subcriber
	...
	scriba_msgs::motion_odom motion_msg;		//Motion publisher message
...
void OdometryBroadcaster::setupSubPub(){
	...
	//Set motion message publisher
	motion_pub = nh.advertise<scriba_msgs::motion_odom>("odometry_motion", 50);
...
void OdometryBroadcaster::odomDataCallback(const scriba_msgs::data_odom& odom_data_msg){
	...
	ROS_INFO_STREAM("Estimated angle is " << phi);
 	...
 	if(phi == 0){ //If no angular displacement
 	...
 	//Publish the motion
 	motion_msg.header = odom_data_msg.header;
 	motion_msg.steerAngle = phi;
 	motion_msg.frontWheelDistance = frontWheelDistance;
 	motion_msg.durationMillis = odom_data_msg.durationMillis;
